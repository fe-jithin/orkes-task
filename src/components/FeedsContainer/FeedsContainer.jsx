...
const FeedsContainer = () => {
    ...
    const loadItems = async ()=>{
        if (isLoading || feedData.length >= 50) {
            return;
        }
        setIsLoading(true);
    ...
    return (
        <div className='feed_wrapper'>
            {
                feedData.map((feed) => {
                    return <FeedItem key={uuidv4()} thumbnail={feed.node.field_photo_image_section} title={feed.node.title} timeStamp={feed.node.last_update} />
                })
            }
